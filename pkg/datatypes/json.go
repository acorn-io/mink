...
// Value extract json with path
func (jsonQuery *JSONQueryExpression) Value(keys ...string) *JSONQueryExpression {
	jsonQuery.value = true
	jsonQuery.keys = keys
	return jsonQuery
}
		...
		case "postgres":
			...
			case jsonQuery.value:
				if len(jsonQuery.keys) > 0 {
					...
					for i, key := range jsonQuery.keys {
			...
			case jsonQuery.equals:
				if len(jsonQuery.keys) > 0 {
					builder.WriteString(fmt.Sprintf("json_extract_path_text(%v::jsonb,", stmt.Quote(jsonQuery.column)))
